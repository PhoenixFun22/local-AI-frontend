...
app.post('/api/auth/signup', async (req, res) => {
  try {
    const { username, firstName, lastName, role, authFile, encryptedChats, encryptedCanvases, encryptedSettings, ageGroup, tetheredParent, parentTetherFile } = req.body;
    ...
    let profileContent = `First Name: ${firstName.trim()}\nLast Name: ${lastName.trim()}\nRole: ${assignedRole}\n`;
    if (ageGroup) {
      profileContent += `Age Group: ${ageGroup}\n`;
    } else {
      profileContent += `Age Group: adult\n`;
    }
    if (tetheredParent) {
      profileContent += `Tethered Parent: ${tetheredParent.trim().toLowerCase()}\n`;
    }
    ...
    if (parentTetherFile) {
      await fs.writeFile(path.join(userDir, 'parent_tether.enc'), parentTetherFile, 'utf-8');
    }
...
app.post('/api/auth/get-profile', async (req, res) => {
  try {
    ...
    for (const line of lines) {
      ...
      if (line.startsWith('Age Group:')) ageGroup = line.replace('Age Group:', '').trim();
      if (line.startsWith('Tethered Parent:')) tetheredParent = line.replace('Tethered Parent:', '').trim();
    }
...
// 4b. Verify parent credentials during child signup
app.post('/api/auth/verify-parent', async (req, res) => {
  try {
    const { parentUsername } = req.body;
    if (!parentUsername) {
      res.status(400).json({ error: 'Parent username is required' });
      return;
    }

    const cleanParent = parentUsername.trim().toLowerCase();
    const parentDir = path.join(STORAGE_DIR, cleanParent);

    if (!existsSync(parentDir)) {
      res.status(404).json({ error: 'Parent user does not exist' });
      return;
    }

    // Read parent profile
    const profileContent = await fs.readFile(path.join(parentDir, 'profile.txt'), 'utf-8');
    const lines = profileContent.split('\n');
    let role = 'user';
    let ageGroup = 'adult';
    let firstName = '';
    ...
    // Only non-kids can tether (Adults, Teens, Admins)
    if (ageGroup === 'kid') {
      res.status(400).json({ error: 'A child account cannot be tethered to another child account' });
      return;
    }

    const authFile = await fs.readFile(path.join(parentDir, 'auth.enc'), 'utf-8');

    res.json({
      username: cleanParent,
      firstName,
      lastName,
      role,
      ageGroup,
...
// 4c. Get list of kids tethered to the logged in parent
app.get('/api/parent/tethered-kids', authenticateToken, async (req, res) => {
  try {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        ...
        if (existsSync(profilePath)) {
          ...
          if (tetheredParent.toLowerCase() === parentUsername) {
            // Read parent_tether.enc
            const tetherPath = path.join(STORAGE_DIR, entry.name, 'parent_tether.enc');
            let parentTetherFile = '';
            if (existsSync(tetherPath)) {
              parentTetherFile = await fs.readFile(tetherPath, 'utf-8');
            }

            // Also read encrypted chats for the kid so parent can decrypt and view!
            const chatsPath = path.join(STORAGE_DIR, entry.name, 'chats.enc');
            let encryptedChats = '';
            if (existsSync(chatsPath)) {
              encryptedChats = await fs.readFile(chatsPath, 'utf-8');
            }
