...
import { Cpu, Sparkles, AlertCircle, Globe, Zap } from 'lucide-react';
...
export default function App() {
  ...
  const [kidStyle, setKidStyle] = useState<string>('candy');

  // Safety filter & Age states
  const [isCheckingSafety, setIsCheckingSafety] = useState<boolean>(false);
  const [isAdultFilterEnabled, setIsAdultFilterEnabled] = useState<boolean>(false);
  ...
  const saveUserData = async (
    ...
    updatedSettings?: { memories: string; systemPrompts: Record<string, string>; demoMode: boolean; isAdultFilterEnabled: boolean; kidStyle?: string }
  ) => {
    ...
    const settingsToEncrypt = {
      ...
      isAdultFilterEnabled: updatedSettings?.isAdultFilterEnabled ?? isAdultFilterEnabled,
      kidStyle: updatedSettings?.kidStyle ?? kidStyle,
    };
  ...
  const handleAuthSuccess = async (authData: {
    ...
    try {
      ...
      if (res.ok) {
        ...
        let loadedSettings: { memories?: string; systemPrompts?: Record<string, string>; demoMode?: boolean; isAdultFilterEnabled?: boolean; kidStyle?: string } | null = null;
        ...
        if (loadedSettings) {
          ...
          setDemoModeState(authData.ageGroup === 'kid' ? false : (loadedSettings.demoMode ?? true));
          setIsAdultFilterEnabled(loadedSettings.isAdultFilterEnabled ?? false);
          setKidStyle(loadedSettings.kidStyle || 'candy');
        } else {
          setMemories('');
          setSystemPrompts({});
          setDemoModeState(authData.ageGroup === 'kid' ? false : true);
          setIsAdultFilterEnabled(false);
          setKidStyle('candy');
        }
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (isDataLoaded && sessionToken && encryptionKey) {
      saveUserData(undefined, undefined, { memories, systemPrompts, demoMode, isAdultFilterEnabled, kidStyle });
    }
  }, [memories, systemPrompts, demoMode, isAdultFilterEnabled, kidStyle, isDataLoaded, sessionToken, encryptionKey]);
...
${newContent}`
      ...
      const isKidMode = user?.ageGroup === 'kid';
      const isFilterActive = isKidMode || isAdultFilterEnabled;
      ...
      if (isFilterActive) {
        ...
        try {
          ...
          if (demoMode) {
            ...
            generatedText = getSimulatedReply(activeModel, newContent);
          } else {
            // Call non-streaming real Ollama chat
            ...
            const { fetchOllamaChat } = await import('./utils/ollama');
            generatedText = await fetchOllamaChat(ollamaUrl, activeModel, fullHistory);
          }

          // AI is finished generating, now checking content safety!
          setIsCheckingSafety(true);

          const { checkContentSafety } = await import('./utils/ollama');
          const isSafe = await checkContentSafety(ollamaUrl, activeModel, generatedText, demoMode, user?.ageGroup);

          const finalOutputText = isSafe
            ? generatedText
            : isKidMode
              ? "Oops! 🌟 That prompt triggered my safety filters to keep things clean and friendly. Let's ask something else! 🎈"
              : "This response triggered the model to be unsafe";

          // Display completely finished text after safety evaluation
          setChats(prev => prev.map(c => {
            if (c.id === currentChatId) {
              const updatedMessages = c.messages.map(m => 
                m.id === assistantDraftMessage.id 
                  ? { ...m, content: finalOutputText } 
                  : m
              );
          ...
          setIsCheckingSafety(false);
          triggerAutoTitleHeuristic(currentChatId, newContent, finalOutputText, activeModel);

        } catch (err: any) {
          console.error('Safety flow failed during edit rerun:', err);
          setChats(prev => prev.map(c => {
            if (c.id === currentChatId) {
              return {
                ...c,
                messages: c.messages.map(m => 
                  m.id === assistantDraftMessage.id 
                    ? { ...m, content: `⚠️ **Ollama Error**: ${err.message || 'The connection failed. Please check your settings.'}` } 
                    : m
                ),
              };
            }
...
${promptText}`
    ...
    if (isFilterActive) {
      ...
      try {
        ...
        if (demoMode) {
          // Simulate complete response generation
          await new Promise(resolve => setTimeout(resolve, 1500));
          generatedText = getSimulatedReply(activeModel, promptText);
        } else {
          // Call non-streaming real Ollama chat
          const systemPromptMessages: { role: 'system' | 'user' | 'assistant', content: string }[] = [];
          const modelSystemPrompt = systemPrompts[activeModel] || '';
        ...
        setIsGenerating(false);
        setIsCheckingSafety(false);
        triggerAutoTitleHeuristic(chatId!, promptText, finalOutputText, activeModel);

      } catch (err: any) {
        console.error('Safety flow failed:', err);
        setChats(prev => prev.map(c => {
          if (c.id === chatId) {
            return {
              ...c,
  ...
  return (
    <div 
      id="ollama-app-root" 
      className={`h-screen flex overflow-hidden font-sans animate-in fade-in duration-300 ${
        isKidMode 
          ? `${currentKidTheme.rootBg} ${currentKidTheme.textPrimary}` 
    ...
    >
      ...
      <div className={`flex-1 flex flex-col h-full min-w-0 relative ${isKidMode ? currentKidTheme.mainBg : 'bg-zinc-900/40'}`}>
        
        {/* Navigation / Header bar */}
        <header className={`h-14 border-b px-4 flex items-center justify-between select-none backdrop-blur-md ${
          isKidMode 
            ? `${currentKidTheme.headerBorder} ${currentKidTheme.headerBg}` 
          ...
          <div className="flex items-center gap-3">
            {!isKidMode && (
              <div className="flex items-center bg-zinc-950 border border-zinc-800 rounded-full p-0.5 select-none text-[10px] font-bold">
                <button
                  onClick={() => setDemoMode(false)}
                  className={`px-2.5 py-1 rounded-full flex items-center gap-1 transition-all duration-200 cursor-pointer ${
                    !demoMode
            ...
            <div 
              ...
              className={`flex items-center gap-1.5 rounded-full border px-3 py-1 text-[10px] font-bold cursor-pointer transition select-none ${
                isKidMode
                  ? `${currentKidTheme.badgeBg} text-white`
                  : demoMode 
            ...
            >
              ...
              <span>{isKidMode ? 'Kids Playground Active 🐕🐾' : demoMode ? 'Sandbox Active' : status.connected ? 'Connected' : 'Disconnected'}</span>
            </div>
      ...
      <SettingsModal
        ...
        setIsAdultFilterEnabled={setIsAdultFilterEnabled}
        kidStyle={kidStyle}
        setKidStyle={setKidStyle}
      />
    </div>
  );
}
